...
func systemCollisions(game *SoccerGame) []collision {

	collisions := make([]collision, 0)

	for _, coll := range game.collisionListener.PopCollisions() {
		...
		entityResultA := game.getEntity(A.ID, game.collidableComponent)
		entityResultB := game.getEntity(B.ID, game.collidableComponent)
		...
		compiledCollision := collision{
			...
			point:             vector.FromB2Vec2(worldManifold.Points[0]).Transform(game.physicalToAgentSpaceTransform),
		...
		collidableAspectA.CollisionScript(game, A.ID, B.ID, collidableAspectA, collidableAspectB, compiledCollision.point)
		collidableAspectB.CollisionScript(game, B.ID, A.ID, collidableAspectB, collidableAspectA, compiledCollision.point)
	}
